...
class StravaService(
    ...
    fun toActivitySummary(dto: StravaActivityDto): ActivitySummary = ActivitySummary(
        ...
        distanceKm = if (dto.distance == 0.0) null else (dto.distance / 1000 * 10).roundToInt() / 10.0,
        movingTimeFormatted = formatSeconds(dto.moving_time),
        elevationGain = if (dto.distance == 0.0) null else dto.total_elevation_gain,
    ...
    fun toActivitySummary(activity: Activity): ActivitySummary {
        ...
        return ActivitySummary(
            ...
            distanceKm = if (activity.distanceMeters == 0.0) null else (activity.distanceMeters / 1000 * 10).roundToInt() / 10.0,
            movingTimeFormatted = formatSeconds(activity.movingTimeSecs),
            elevationGain = if (activity.distanceMeters == 0.0) null else activity.totalElevationGain,
